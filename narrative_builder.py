...
class NarrativeBuilder:
    ...
    def build_stock_narrative(
        self, multi_signal: MultiHorizonSignal, level_outcomes: list[PriceLevelOutcome] | None = None
    ) -> str:
        """
        ...
        try:
            ...
            if level_outcomes:
                for outcome in level_outcomes:
                    if outcome.predicted_level != "NONE":
                        level_parts.append(
                            f"In the {outcome.horizon} window, price is most likely to test {outcome.predicted_level} first (Confidence: {outcome.confidence*100:.1f}%)."
                        )

            level_summary = (
                " ".join(level_parts) if level_parts else "No significant price level tests are immediately projected."
            )
            ...
            if primary_sig and primary_sig.contributing_events:
                event_summary = (
                    "Key driving events include: "
                    + "; ".join([e.headline_or_label for e in primary_sig.contributing_events[:3]])
                    + "."
                )
            else:
...
if __name__ == "__main__":
    ...
    # Mock some data
    from config import HORIZON_30D, HORIZON_INTRADAY
