...
class ReferenceLevelEngine:
    ...
    def get_reference_levels(
        ...
        try:
            ...
            return ReferenceLevels(
                ...
                moving_average_value=current,
                moving_average_period=1,
                support_band_low=current,
                support_band_high=current,
                resistance_band_low=current,
                resistance_band_high=current,
                user_avg_cost=None,
            )
    ...
    def compute_deltas(self, levels: ReferenceLevels) -> ReferenceLevelDeltas:
        ...
        pct_from_ma = (
            ((cp - levels.moving_average_value) / levels.moving_average_value) * 100.0
            if levels.moving_average_value > 0
            else 0.0
        )
